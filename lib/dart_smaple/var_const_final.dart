...
//  const timec = new DateTime.now();
  ...
  const constListError = [1, 2, 3]; //error  error个鸡毛 我实测不会有问题呀
  const constListOk = const [1, 2, 3]; //ok
  print(constListError);
  print(constListOk);
//  print(constListError);
  var list = [1, 2, 3];
  list.forEach(printElement); //输出1，2，3
  var callbacks = [];
  for (int i = 0; i < 3; i++) {
    //存入一个方法作用是输出save i
    callbacks.add(() => print('save $i'));
  }
  callbacks.forEach((a) => a()); //输出 save 1 svae 2 save 3
  var a = 10;
  if (a is int) {
    print('main' + "true"); //输出maintrue
  }
  String str1 = 'my'
      'is'
      "your father";
...
//      print(it.current);
  ...
  const list2 = const [1, 2, 3]; // Ok
  const list3 = [1, 2, 3]; // Error

  final list4 = [1, 2, 3]; // Ok
  final list5 = const [1, 2, 3]; // Ok
//  final list6 = const [new DateTime.now(), 2, 3]; // Error,const右边必须是常量
}
